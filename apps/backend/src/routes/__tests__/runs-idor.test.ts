/**
 * IDOR tests for PUT /api/runs/:id and DELETE /api/runs/:id
 *
 * Verifies that a user cannot modify or delete a run belonging to another user.
 */

import { describe, it, expect, vi, beforeAll, afterAll, beforeEach } from 'vitest';
import http from 'node:http';
import jwt from 'jsonwebtoken';

// ── Environment stubs ─────────────────────────────────────────────────────────
process.env.JWT_SECRET = 'test-jwt-secret-for-integration-tests';
process.env.SUPABASE_URL = 'http://localhost:54321';
process.env.SUPABASE_SERVICE_ROLE_KEY = 'test-service-role-key';

// ── Mock the database layer ───────────────────────────────────────────────────
vi.mock('../../config/database.js', () => ({
  default: {},
  supabase: { client: {} },
  testDatabaseConnection: vi.fn(),
  getSupabaseClient: vi.fn(),
}));

// ── Mock XP calculation ───────────────────────────────────────────────────────
vi.mock('../../utils/xpCalculation.js', () => ({
  metersToKm: (m: number) => m / 1000,
  getLevelFromXP: vi.fn().mockResolvedValue(1),
  getXPForLevel: vi.fn().mockResolvedValue(0),
  getLevelProgress: vi.fn().mockResolvedValue({ level: 1, progress: 0 }),
}));

// ── Mock @runquest/shared ─────────────────────────────────────────────────────
vi.mock('@runquest/shared', () => ({
  calculateCompleteRunXP: vi.fn().mockReturnValue({
    baseXP: 15, kmXP: 10, distanceBonus: 5, streakBonus: 0,
    multiplier: 1.0, finalXP: 30,
  }),
  calculateRunXP: vi.fn().mockReturnValue({ baseXP: 15, kmXP: 10, distanceBonus: 5, totalXP: 30 }),
  calculateStreakMultiplier: vi.fn().mockReturnValue(1.0),
}));

// ── Mock calculateUserTotals ──────────────────────────────────────────────────
vi.mock('../../utils/calculateUserTotals.js', () => ({
  calculateUserTotals: vi.fn().mockResolvedValue(undefined),
}));

import { getSupabaseClient } from '../../config/database.js';
import app from '../../app.js';

// ── HTTP helpers ──────────────────────────────────────────────────────────────

let server: http.Server;
let port: number;

function httpRequest(
  method: 'PUT' | 'DELETE',
  path: string,
  body: unknown = null,
  headers: Record<string, string> = {}
): Promise<{ status: number; body: any }> {
  return new Promise((resolve, reject) => {
    const payload = body ? JSON.stringify(body) : '';
    const req = http.request(
      {
        hostname: '127.0.0.1',
        port,
        path,
        method,
        headers: {
          'Content-Type': 'application/json',
          'Content-Length': Buffer.byteLength(payload),
          ...headers,
        },
      },
      (res) => {
        let raw = '';
        res.on('data', (chunk) => { raw += chunk; });
        res.on('end', () => {
          try {
            resolve({ status: res.statusCode ?? 0, body: JSON.parse(raw) });
          } catch {
            resolve({ status: res.statusCode ?? 0, body: raw });
          }
        });
      }
    );
    req.on('error', reject);
    if (payload) req.write(payload);
    req.end();
  });
}

const put = (path: string, body: unknown, headers?: Record<string, string>) =>
  httpRequest('PUT', path, body, headers);
const del = (path: string, headers?: Record<string, string>) =>
  httpRequest('DELETE', path, null, headers);

beforeAll(() => new Promise<void>((resolve) => {
  server = http.createServer(app);
  server.listen(0, '127.0.0.1', () => {
    port = (server.address() as any).port;
    resolve();
  });
}));

afterAll(() => new Promise<void>((resolve) => {
  server.close(() => resolve());
}));

// ── Helpers ───────────────────────────────────────────────────────────────────

function mintToken(userId = 'user-uuid-test') {
  return jwt.sign(
    { user_id: userId, name: 'TestUser', email: 'test@example.com' },
    process.env.JWT_SECRET as string,
    { expiresIn: '1h' }
  );
}

/** Stub where the run exists and belongs to `ownerUserId`. */
function buildRunOwnershipStub(ownerUserId: string) {
  const run = { user_id: ownerUserId, date: '2025-08-01', distance: 5.0, streak_day: 1 };
  return {
    from: vi.fn().mockReturnThis(),
    select: vi.fn().mockReturnThis(),
    insert: vi.fn().mockReturnThis(),
    update: vi.fn().mockReturnThis(),
    delete: vi.fn().mockReturnThis(),
    eq: vi.fn().mockReturnThis(),
    lt: vi.fn().mockReturnThis(),
    gte: vi.fn().mockReturnThis(),
    order: vi.fn().mockReturnThis(),
    limit: vi.fn().mockResolvedValue({ data: [], error: null }),
    single: vi.fn().mockResolvedValue({ data: run, error: null }),
  };
}

/** Stub where the run does not exist. */
function buildRunNotFoundStub() {
  return {
    from: vi.fn().mockReturnThis(),
    select: vi.fn().mockReturnThis(),
    update: vi.fn().mockReturnThis(),
    delete: vi.fn().mockReturnThis(),
    eq: vi.fn().mockReturnThis(),
    order: vi.fn().mockReturnThis(),
    limit: vi.fn().mockResolvedValue({ data: [], error: null }),
    single: vi.fn().mockResolvedValue({ data: null, error: { message: 'not found', code: 'PGRST116' } }),
  };
}

// ─────────────────────────────────────────────────────────────────────────────

describe('PUT /api/runs/:id', () => {
  const token = mintToken('user-uuid-test');
  const authHeader = { Authorization: `Bearer ${token}` };

  beforeEach(() => { vi.clearAllMocks(); });

  it('returns 401 when no Authorization header is provided', async () => {
    const { status } = await put('/api/runs/run-uuid-1', { distance: 6.0 });
    expect(status).toBe(401);
  });

  it('returns 403 when the token is invalid', async () => {
    const { status } = await put(
      '/api/runs/run-uuid-1',
      { distance: 6.0 },
      { Authorization: 'Bearer not-a-valid-token' }
    );
    expect(status).toBe(403);
  });

  it('returns 400 when distance is missing', async () => {
    vi.mocked(getSupabaseClient).mockReturnValue(buildRunOwnershipStub('user-uuid-test') as any);
    const { status, body } = await put('/api/runs/run-uuid-1', {}, authHeader);
    expect(status).toBe(400);
    expect(body.error).toMatch(/distance is required/i);
  });

  it('returns 404 when the run does not exist', async () => {
    vi.mocked(getSupabaseClient).mockReturnValue(buildRunNotFoundStub() as any);
    const { status, body } = await put('/api/runs/nonexistent-run', { distance: 6.0 }, authHeader);
    expect(status).toBe(404);
    expect(body.error).toMatch(/run not found/i);
  });

  it('returns 403 (IDOR) when run belongs to a different user', async () => {
    vi.mocked(getSupabaseClient).mockReturnValue(buildRunOwnershipStub('other-user-uuid') as any);
    const { status, body } = await put('/api/runs/run-uuid-other', { distance: 6.0 }, authHeader);
    expect(status).toBe(403);
    expect(body.error).toMatch(/not authorized/i);
  });

  it('returns 200 when updating your own run', async () => {
    vi.mocked(getSupabaseClient).mockReturnValue(buildRunOwnershipStub('user-uuid-test') as any);
    const { status, body } = await put('/api/runs/run-uuid-1', { distance: 6.0 }, authHeader);
    expect(status).toBe(200);
    expect(body.success).toBe(true);
  });
});

// ─────────────────────────────────────────────────────────────────────────────

describe('DELETE /api/runs/:id', () => {
  const token = mintToken('user-uuid-test');
  const authHeader = { Authorization: `Bearer ${token}` };

  beforeEach(() => { vi.clearAllMocks(); });

  it('returns 401 when no Authorization header is provided', async () => {
    const { status } = await del('/api/runs/run-uuid-1');
    expect(status).toBe(401);
  });

  it('returns 403 when the token is invalid', async () => {
    const { status } = await del(
      '/api/runs/run-uuid-1',
      { Authorization: 'Bearer not-a-valid-token' }
    );
    expect(status).toBe(403);
  });

  it('returns 404 when the run does not exist', async () => {
    vi.mocked(getSupabaseClient).mockReturnValue(buildRunNotFoundStub() as any);
    const { status, body } = await del('/api/runs/nonexistent-run', authHeader);
    expect(status).toBe(404);
    expect(body.error).toMatch(/run not found/i);
  });

  it('returns 403 (IDOR) when run belongs to a different user', async () => {
    vi.mocked(getSupabaseClient).mockReturnValue(buildRunOwnershipStub('other-user-uuid') as any);
    const { status, body } = await del('/api/runs/run-uuid-other', authHeader);
    expect(status).toBe(403);
    expect(body.error).toMatch(/not authorized/i);
  });

  it('returns 200 when deleting your own run', async () => {
    vi.mocked(getSupabaseClient).mockReturnValue(buildRunOwnershipStub('user-uuid-test') as any);
    const { status, body } = await del('/api/runs/run-uuid-1', authHeader);
    expect(status).toBe(200);
    expect(body.success).toBe(true);
    expect(body.message).toMatch(/deleted/i);
  });
});
