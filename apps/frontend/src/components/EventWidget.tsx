...
export const EventWidget: React.FC<Props> = ({ data }) => {
  ...
  const scheduled = data.kind === 'participation' ? data.scheduled : data.scheduled;
  ...
  return (
    ...
    >
      {data.kind === 'participation' ? (
        <div className={`flex flex-col gap-1 ${scheduled ? 'opacity-40' : ''}`}>
          <div className="flex items-center justify-between gap-2">
            ...
            {data.done
              ...
              : !scheduled && <span className="text-xs font-bold shrink-0" style={{ color: 'var(--rq-gold)' }}>+{data.rewardXp} XP</span>
            }
          </div>
          {data.done
        ...
        <div className={`flex flex-col gap-1 ${scheduled ? 'opacity-40' : ''}`}>
          <div className="flex items-center justify-between gap-2">
            ...
            {!scheduled && <span className="text-xs text-muted-foreground shrink-0">{formatDaysLeft(data.endsAt)}</span>}
          </div>
          {!scheduled && (
      ...
      {/* Pending overlay — same style as ActiveChallengeWidget */}
      {scheduled && (
